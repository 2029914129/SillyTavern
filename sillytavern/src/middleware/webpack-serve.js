...
export default function getWebpackServeMiddleware() {
    const usePrecompiledBundle = process.env.ST_ANDROID === '1';
    ...
    function devMiddleware(req, res, next) {
        const publicLibConfig = getPublicLibConfig({ forceDist: usePrecompiledBundle });
    ...
    devMiddleware.runWebpackCompiler = ({ forceDist = false, pruneCache = false } = {}) => {
        ...
        const publicLibConfig = getPublicLibConfig({
            forceDist: forceDist || usePrecompiledBundle,
            pruneCache: usePrecompiledBundle ? false : pruneCache,
        });
        const outputPath = publicLibConfig.output?.path;
        const outputFile = publicLibConfig.output?.filename;
        const precompiledPath = outputPath && outputFile ? path.join(outputPath, outputFile) : null;

        if (usePrecompiledBundle) {
            if (!precompiledPath || !fs.existsSync(precompiledPath)) {
                throw new Error(`Precompiled frontend bundle is missing: ${precompiledPath ?? 'unknown path'}`);
            }
            console.log(`Using precompiled frontend libraries: ${precompiledPath}`);
